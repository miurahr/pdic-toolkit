  ...
  <span>
    <xsl:choose>
      <xsl:when test="level = 0">
        <xsl:attribute name="style">color:black;font-size:120%</xsl:attribute>
      </xsl:when>
      <xsl:when test="level = 1">
        <xsl:attribute name="style">color:#2222cc;font-size:140%</xsl:attribute>
      </xsl:when>
      <xsl:when test="level = 2">
        <xsl:attribute name="style">color:#cc2222;font-size:160%</xsl:attribute>
      </xsl:when>
      <xsl:otherwise>
        <xsl:attribute name="style">color:#cc2222;font-size:200%</xsl:attribute>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:value-of select="$hw"/>
  </span>
  </b>
  &#x20;<span style="color:#cc6666;font-size:90%;font-style:italic"><xsl:value-of select="$pos"/></span>
  &#x20;<xsl:value-of select="definition"/>
  &#x20;<span style="color:#66cccc;font-size:90%;font-style:italic"><xsl:value-of select="$inflection"/></span>
  &#x20;<span style="color:#cc66cc;font-size:90%;font-style:italic"><xsl:value-of select="$reference"/></span>
</div>
</xsl:template>
